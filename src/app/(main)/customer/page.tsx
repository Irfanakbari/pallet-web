...
import AddModalLayout from "@/app/components/Pages/Master/Customer/AddModal";
import EditableCell from "@/app/components/Pages/Master/Customer/EditCell";
...
export default function Customer() {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const response = await axiosInstance.get('/pallet/customers',{
        id: 'list-customer',
      });
  ...
  const submitData = async (data: any) => {
    try {
      await axiosInstance.post('/pallet/customers', {
        name: data.name,
        kode: data.kode
      },{
        cache: {
  ...
  const deleteData = async (e: string) => {
    ...
    try {
      await axiosInstance.delete(`/pallet/customers/${e}`,{
        cache: {
          update: {
            'list-customer': 'delete'
          }
  ...
  const save = async (key: any) => {
    try {
      ...
      if (index > -1) {
        ...
        await axiosInstance.patch(`/pallet/customers/${item.kode}`, row, {
          cache: {
            update: {
              'list-customer': 'delete'
            }
